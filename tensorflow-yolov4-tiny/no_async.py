...
def build_argparser():
    ...
    args.add_argument("-m", "--model", default='yolov4-tiny-obj_best.xml',
...
#                         373.0, 326.0] if 'anchors' not in param else [float(a) for a in param['anchors'].split(',')]
        self.anchors = [10.0,14.0,  23.0,27.0,  37.0,58.0,  81.0,82.0,  135.0,169.0,  
                        344.0,319.0] if 'anchors' not in param else [float(a) for a in param['anchors']]
...
if __name__ == '__main__':
    ...
    elif args.input in IMAGE_EXTS and os.path.isfile(args.input):
    ...
    inference_times = []
    parsing_times = []
    while cap.isOpened() or index_images != 0:
        if len(input_images) != 0:
            img_name = input_images[index_images - 1]
        else:
            ret, frame = cap.read()
            img_name = ("result_{}.jpg".format(count_frame))
        if not ret:
            break
        ...
        if key == 27:
            break
    print("inference_time average is {}". format(np.mean(inference_times), np.std(inference_times)))
    print("parsing_time average is {}". format(np.mean(parsing_times), np.std(parsing_times)))
