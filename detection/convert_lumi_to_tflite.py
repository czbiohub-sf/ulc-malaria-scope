...
def convert_lumi_to_tflite(model, config_file, checkpoint, output_dir):
    ...
    with tf.Session() as sess:
        sess.run([slot_init, tf.global_variables()])
        # Restore variables from disk.
        saver.restore(sess, checkpoint)
        print("Model restored.")
        builder.add_meta_graph_and_variables(
            sess, ["tfckpt2pb"], strip_default_attrs=False
        )
        builder.save()
...
def main():
    ...
    convert_lumi_to_tflite(
        args.model, args.config_file, args.checkpoint, os.path.abspath(args.output_dir)
    )
