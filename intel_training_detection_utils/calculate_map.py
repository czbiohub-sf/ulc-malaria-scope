...
def get_parser():
    ...
    parser.add_argument(
        "--data",
        help="Path to .data file used in training",
        type=str)
    parser.add_argument(
        "--config",
        help="Path to config file used to train the model",
        type=str)
    parser.add_argument(
        "--weights_location",
...
def calculate_map():
    ...
    weights_files = glob.glob(
        os.path.join(args.weights_location, '*000.weights'))
    weights_files.sort(key=os.path.getmtime)
    print(weights_files)
    for path in weights_files:
        path_without_extension = path.split('.')[0]
        command = "./darknet detector map {} {} {} > {}.map".format(args.data, args.config, path, path_without_extension)
        os.system(command)
