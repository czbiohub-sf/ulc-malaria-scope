...
def build_argparser():
    ...
    args.add_argument(
        "-i",
        "--input",
        help="Required. Path to an image/video file. (Specify 'cam' to work with "
        "camera)",
        required=True,
        type=str,
    )
    ...
    args.add_argument(
        "--labels", help="Optional. Labels mapping file", default=None, type=str
    )
    args.add_argument(
        "-t",
        "--prob_threshold",
    ...
    args.add_argument(
        "-loop_input",
        "--loop_input",
        help="Optional. Iterate over input infinitely",
        action="store_true",
    )
    args.add_argument(
        "-no_show", "--no_show", help="Optional. Don't show output", action="store_true"
    )
    args.add_argument(
        "-u",
...
def intersection_over_union(box_1, box_2):  # add DIOU-NMS support
    ...
    rh02 = (
        (box_2["xmax"] + box_2["xmin"]) - (box_1["xmax"] + box_1["xmin"])
    ) ** 2 / 4 + (
        (box_2["ymax"] + box_2["ymin"]) - (box_1["ymax"] + box_1["ymin"])
    ) ** 2 / 4
...
def main():
    ...
    assert (
        len(net.input_info) == 1
    ), "Sample supports only YOLO V3 based single input topologies"
    ...
    while (
        ...
        if next_frame_id_to_show in completed_request_results:
            ...
            for obj in objects:
                ...
                color = (
                    min(obj["class_id"] * 12.5, 255),
                    min(obj["class_id"] * 7, 255),
                    min(obj["class_id"] * 5, 255),
                )
                det_label = (
                    labels_map[obj["class_id"]]
                    if labels_map and len(labels_map) >= obj["class_id"]
            ...
            in_frame = preprocess_frame(
                frame, input_height, input_width, nchw_shape, args.keep_aspect_ratio
            )
    ...
    for mode_value in mode_info.keys():
        ...
        end_time = (
            mode_info[mode_value].last_end_time
            if mode_value in mode_info
            and mode_info[mode_value].last_end_time is not None
            else perf_counter()
        )
        log.info(
            "FPS: {:.1f}".format(
                mode_info[mode_value].frames_count
                / (end_time - mode_info[mode_value].last_start_time)
            )
        )
        log.info(
            "Latency: {:.1f} ms".format(
                (mode_info[mode_value].latency_sum / mode_info[mode_value].frames_count)
                * 1e3
            )
        )
