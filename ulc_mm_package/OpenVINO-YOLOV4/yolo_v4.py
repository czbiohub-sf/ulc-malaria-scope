...
def _fixed_padding(inputs, kernel_size, *args, mode="CONSTANT", **kwargs):
    ...
    if kwargs["data_format"] == "NCHW":
        padded_inputs = tf.pad(
            inputs, [[0, 0], [0, 0], [pad_beg, pad_end], [pad_beg, pad_end]], mode=mode
        )
    else:
        padded_inputs = tf.pad(
...
def yolo_v4(inputs, num_classes, is_training=False, data_format="NCHW", reuse=False):
    ...
    with slim.arg_scope(
        [slim.conv2d, slim.batch_norm, _fixed_padding],
        ...
        with tf.variable_scope("cspdarknet-53"):
            route_1, route_2, route_3 = csp_darknet53(
                inputs, data_format, batch_norm_params
            )
