...
def takeZStack(
    camera: ULCMM_Camera, motor: DRV8825Nema, steps_per_image: int = 1, save_images=True
):
...
def takeZStackCoroutine(
    img, motor: DRV8825Nema, steps_per_coarse: int = 10, steps_per_fine: int = 1
):
    # Re-home the motor to the limit switch
    motor.homed = False
    motor.homeToLimitSwitches()
...
if __name__ == "__main__":
    ...
    most_focused, metrics = symmetricZStack(
        camera=camera, motor=motor, start_point=450, save_images=True
    )
    print(f"\n=======Most focused image is likely: {most_focused:03d}.tiff=======\n")

    plt.plot(
        range(0, int(motor.max_pos), steps_per_image),
        metrics,
        "o",
        markersize=2,
        color="#2CBDFE",
