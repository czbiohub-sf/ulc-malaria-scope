...
def get_parser():
    ...
    parser.add_argument(
        "-i", "--input", help="Path to folder containing flowcell images", type=str
    )
    parser.add_argument(
        "--image_format",
    ...
    parser.add_argument("--min_hue", help="Minimum hue", type=int, default=MIN_HUE)
    parser.add_argument("--max_hue", help="Maximum hue", type=int, default=MAX_HUE)
    return parser


CROP_BOUNDARY_OFFSET = 300
NUM_ROWS = 8
NUM_COLS = 4


def img_to_grid(img, row=NUM_ROWS, col=NUM_COLS):
    print(img.shape)
    img = img[CROP_BOUNDARY_OFFSET : img.shape[0] - CROP_BOUNDARY_OFFSET, :, :]
...
def binarize_by_hue_filter(rgb_flowcell_image, min_hue, max_hue):
    """
...
def count_regions_in_center_of_image(rgb_flowcell_image, binary_flowcell_image):
    ...
    central_subset = binary_flowcell_image[
        center_x - 300 : center_x + 300, center_y - 300 : center_y + 300
    ]
    _, objs = ndimage.label(central_subset)
    central_subset_rgb = rgb_flowcell_image[
        center_x - 200 : center_x + 200, center_y - 200 : center_y + 200, :
    ]
    return central_subset_rgb, central_subset, objs


def write_xlsx(
    path, spacing, subset_images, central_subset_rgbs, num_inference_objects
):
    ...
    rowy = 0
    for binary_image, rgb_image, num_inference_object in zip(
        subset_images, central_subset_rgbs, num_inference_objects
    ):
        print(rowy)
        ...
        print(binary_color_image.shape)
        print(binary_image.shape)
        binary_color_image[binary_image != 0] = [255, 0, 0]
        overlaid_image = cv2.addWeighted(
            rgb_image, ALPHA, binary_color_image, 1 - ALPHA, 0
        )
        temp_overlaid_image = os.path.join(
            temp_folder, "temp_{}_overlaid.png".format(rowy)
        )
...
def main():
    ...
    for image in images:
        print(image)
        rgb_flowcells_image = cv2.cvtColor(cv2.imread(image), cv2.COLOR_BGR2RGB)
        grid = img_to_grid(rgb_flowcells_image)
        for index, each_flowcell_image in enumerate(grid):
            print(index, each_flowcell_image.shape)
            binary_image = binarize_by_hue_filter(each_flowcell_image, min_hue, max_hue)
            print(binary_image.shape)
            (
                central_subset_rgb,
                central_subset,
                num_inference_object,
            ) = count_regions_in_center_of_image(each_flowcell_image, binary_image)
    ...
    write_xlsx(
        path, XLSX_SPACING, central_subsets, central_subset_rgbs, num_inference_objects
    )
