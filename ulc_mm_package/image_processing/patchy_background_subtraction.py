...
class PatchyBackgroundSubtraction():
    ...
    def _maskBoxedRegions(self, img_arr: np.ndarray, xmin_vals: List[int], xmax_vals: List[int], ymin_vals: List[int], ymax_vals: List[int]):
...
class PatchyBackgroundSubtractionFixedNum(PatchyBackgroundSubtraction):
    ...
    def addImage(self, img_arr, xmin_vals, xmax_vals, ymin_vals, ymax_vals):
        ...
        """
        masked_img = self._maskBoxedRegions(img_arr, xmin_vals, xmax_vals, ymin_vals, ymax_vals)
    ...
    def _updateBackgroundAverage(self, masked_img):
        ...
        subtract_step = np.divide((oldest_frame - self._backgroundAverageArray), self.img_counter, where=self.img_counter != 0)
        ...
        add_step = np.divide((masked_img - self._backgroundAverageArray), self.img_counter, where=self.img_counter != 0)
