...
def detect_images(
    ...
    progress_counter = 0
    inference_times = [None]*num_images
    for i, input_image in enumerate(input_images):
        ...
        progress_counter += 1
        if progress_counter % int(0.01*num_images) == 0:
            print(f"Completed: {progress_counter}/{num_images} ({progress_counter/num_images*100:.1f}%), {np.average(inference_times[:i])}")
        # if overlaid:
...
def main():
    ...
    parser.add_argument(
        "-f", "--format", type=str, default=DEFAULT_IMAGE_FORMAT, help="Format of image"
    )
    parser.add_argument(
        "--edgetpu",
