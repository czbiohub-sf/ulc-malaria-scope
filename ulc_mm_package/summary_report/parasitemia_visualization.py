...
def make_parasitemia_plot(parasitemia, err, savefile):
    ...
    parasitemia_raw, bounds_raw, parasitemia_plot, bounds_plot = format_input(
        parasitemia, err
    )
    ...
    gradient_len = 1000
    gradient = np.hstack(
        (np.ones(int(gradient_len / 14)), np.linspace(2, 10, gradient_len))
    )
    gradient = np.vstack((gradient, gradient))

    ax0.imshow(
        gradient, aspect="auto", extent=(0, 1, -4.25, 6.25), cmap=cmap
    )  # extent manually selected to fit
    ...
    ax1.text(
        bounds_plot[1] * 1.1, 6.5, bounds_raw[1], ha="left", va="bottom", alpha=0.7
    )
    ...
    rect = patches.Rectangle(
        (0.8, -5), 0.2, 80, linewidth=1, edgecolor="none", facecolor="white", zorder=1
    )
    ax1.add_patch(rect)
