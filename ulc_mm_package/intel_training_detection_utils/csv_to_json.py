...
def convert_csv_to_json(csv_path, labels, images, label_ids, image_ids):
                ...
                annotations.append(
                    {
                        ...
                        "segmentation": None,
                        "bbox": [float(line[1]), float(line[3]), width, height],
                        "iscrowd": 0,
                        "area": width * height,
                        "id": 0,
...
def main():
    ...
    train_images, train_image_ids = get_images_dict(
        os.path.join(args.input, "train"), args.image_format, "train", width, height
    )
    val_images, val_image_ids = get_images_dict(
        os.path.join(args.input, "val"), args.image_format, "val", width, height
    )
    ...
    convert_csv_to_json(val_csv_path, labels, val_images, label_ids, val_image_ids)
