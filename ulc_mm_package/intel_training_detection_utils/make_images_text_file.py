...
def get_parser():
    ...
    parser.add_argument(
        "--image_format", help="image format of images, including .", type=str
    )
    parser.add_argument("--output_text_path", help="output text file", type=str)
    return parser


def main():
    args = get_parser().parse_args()
    images = glob.glob(os.path.join(os.path.join(args.input), "*" + args.image_format))
