...
def get_parser():
    ...
    parser.add_argument("--labels", help="Labels mapping file", type=str)
    parser.add_argument("--images_text_path", help="Images text file", type=str)
    parser.add_argument(
        "-r",
        "--resolution",
...
def convert_text_to_csv(input, label_names, resolution, image_format, images_text_path):
    images = glob.glob(os.path.join(os.path.join(input), "*" + image_format))
    ...
    for img_name in images:
        if img_name + "\n" in images_from_text:
            ...
            text_path = os.path.join(dirname, basename.replace(image_format, ".txt"))
