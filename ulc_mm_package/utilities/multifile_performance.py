...
if __name__ == "__main__":
    if len(sys.argv) not in [2, 3]:
        print(f"usage: {sys.argv[0]} <path to SSD> [save]")
        sys.exit(1)
    ...
    excluded_keys = ["im_counter", "timestamp", "pressure_status_flag"]
    filtered_keys = [key for key in PER_IMAGE_METADATA_KEYS if key not in excluded_keys]
    ...
    with open(filepath, "r") as f:
        ...
        for row in r:
            for k in run_stats:
                try:
                    if row[k] != "":
                        run_stats[k].append(float(row[k]))
